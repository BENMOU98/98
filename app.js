...
async function generateFacebookContent(recipeIdea, imageUrl = null) {
  ...
  const recipeResponse = await callOpenAI(
    '', // No system prompt in the original code
    replaceVars(config.prompts.fbPrompt, {
      recipeIdea,
      language: config.language
    })
  );
  
  // Clean the recipe text to remove markdown and format consistently
  const recipe = cleanRecipeText(recipeResponse);
  
  // Extract title
  const titleLine = recipe.split('\n')[0];
  const title = titleLine.replace(/[*#]/g, '').trim();
  
  // Extract ingredients
  const ingredientsSection = extractSection(recipe, 'INGREDIENTS', 'INSTRUCTIONS');
  let ingredientsList = [];
  let allIngredients = '';
  let firstThreeIngredients = '';
  
  if (ingredientsSection) {
    // Split into lines and clean up each ingredient
    ingredientsList = ingredientsSection.split('\n')
      .map(line => line.trim())
      .filter(line => line && line !== 'INGREDIENTS')
      .map(line => {
        // Remove bullet points or numbers for consistency
        return line.replace(/^[•\-\d\.]+\s*/, '').trim();
      });
    
    // Format ingredients for Midjourney
    allIngredients = formatIngredientsForMidjourney(ingredientsList);
    firstThreeIngredients = ingredientsList.slice(0, 3).join('\n');
  }
  
  // Extract preparation steps
  const instructionsSection = extractSection(recipe, 'INSTRUCTIONS');
  let instructionsList = [];
  
  if (instructionsSection) {
    // Split into lines and clean up each instruction
    instructionsList = instructionsSection.split('\n')
      .map(line => line.trim())
      .filter(line => line && line !== 'INSTRUCTIONS')
      .map(line => {
        // Remove numbering for consistency
        return line.replace(/^[•\-\d\.]+\s*/, '').trim();
      });
  }
  
  // Store the recipe details in the shared state
  sharedRecipeState = {
    title,
...
if (imageUrl && imageUrl.trim() !== '') {
  ...
  try {
    if (config.enableDiscord) {
      await sendDiscordNotification('recipe_generated', {
        recipeIdea,
        title: facebookContent.title,
        category: 'Facebook Recipe',
        type: 'facebook'
      });
    }
  } catch (discordError) {
    console.warn('Discord notification failed:', discordError.message);
...
async function generateFacebookContent(recipeIdea, imageUrl = null) {
  ...
  // Extract title
  const titleWithEmoji = recipe.split('\n')[0].trim();
  const title = titleWithEmoji.replace(/[\u2700-\u27BF\uE000-\uF8FF\uD800-\uDFFF]+/g, '').trim();
  
  // Extract ingredients
  const ingredientsRaw = extractIngredientsSection(recipe);
  let allIngredients = '';
  let firstThreeIngredients = '';
  let ingredientsList = [];
  
  if (ingredientsRaw) {
    const all = ingredientsRaw.split('\n')
      .map(line => line.replace(/^[-•🧂\s]*|[🧂]$/g, '').trim())
      .filter(line => line && line.length > 1 && !line.match(/^([🧂#*]+)$/));
    
    // Use our new formatting function for Midjourney
    allIngredients = formatIngredientsForMidjourney(all);
    firstThreeIngredients = all.slice(0, 3).join('\n');
    ingredientsList = all;
  }
  
  // Extract preparation steps
  const preparationRaw = extractPreparationSection(recipe);
  let instructionsList = [];
  
  if (preparationRaw) {
    instructionsList = preparationRaw.split('\n')
...
async function generateFacebookContent(recipeIdea, imageUrl = null) {
  ...
  return {
    ...
    title,
    ingredients: firstThreeIngredients,
    allIngredients,  // This contains ONLY the ingredients, not preparation steps
    ingredientsList,
    instructionsList,
    mjPrompt,
    fbCaption,
...
// Updated updateConfig function in app.js to validate prompts
...
function updateConfig(newConfig) {
  ...
  console.log('Updating config with:', {
    ...
    discordEnabled: newConfig.enableDiscord || false
  });
  ...
  console.log('Configuration updated successfully. API Key:', config.apiKey ? 'Set' : 'Not set');
  console.log('Discord integration:', config.enableDiscord ? 'Enabled' : 'Disabled');
}
...
// Function to send Discord notification when content is generated

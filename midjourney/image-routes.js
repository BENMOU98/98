...
function getDiscordSettingsFromApp(req = null) {
  try {
    if (global.getCurrentDiscordSettings) {
      return global.getCurrentDiscordSettings(req);
    }
...
router.post('/api/generate-with-prompt', async (req, res) => {
  try {
    ...
    const discordSettings = getDiscordSettingsFromApp(req);
...
router.get('/api/test-discord-setup', async (req, res) => {
  try {
    ...
    let discordSettings = getDiscordSettingsFromApp(req);
...
// GET /api/recipe/:recipeId - Get images for a specific recipe

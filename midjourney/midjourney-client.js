...
class MidjourneyClient {
  ...
  performFinalSafetyCheck(promptText) {
    ...
    const highRiskTerms = [
      'breast', 'breasts', 'thigh', 'thighs', 'naked', 'nude', 'sexy', 'sexual',
      'blood', 'violence', 'drug', 'porn', 'hate', 'racist', 'kill', 'death',
...
// Export singleton with enhanced functionality
let instance = null;
  ...
  getInstance: (discordSettings = null) => {
    if (!instance) {
      ...
      if (discordSettings && discordSettings.discordChannelId && discordSettings.discordUserToken) {
        ...
        console.log('✅ Using Discord settings from parameter (enhanced)');
      } else {
        if (global.getCurrentDiscordSettings) {
          ...
          if (globalSettings && globalSettings.discordChannelId && globalSettings.discordUserToken) {
            ...
            console.log('✅ Using Discord settings from global helper (enhanced)');
          }
        }
        ...
        if (!channelId || !userToken) {
          ...
          if (channelId && userToken) {
            console.log('⚠️ Using Discord settings from environment variables (enhanced fallback)');
          }
        }
      }
      
      if (!channelId || !userToken) {
        throw new Error('Enhanced Discord settings not configured. Please set Discord Channel ID and User Token in your application settings.');
      }

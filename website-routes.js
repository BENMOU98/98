...
// Add this route to website-routes.js
...
router.get('/website-select', isAuthenticated, async (req, res) => {
  try {
    ...
    // Get websites this user has access to
    let websites;
    ...
    if (req.session.user.role === 'admin') {
      ...
      websites = await new Promise((resolve, reject) => {
        db.all(
          `SELECT * FROM websites WHERE organization_id = ?`,
          [req.session.user.organizationId],
          (err, rows) => {
            if (err) {
              reject(err);
      ...
      // Employees only see websites they have permission for
      websites = await new Promise((resolve, reject) => {
        db.all(
